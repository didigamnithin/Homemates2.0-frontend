...
const ShaderButton = ({ className = '', width, height }: ShaderButtonProps) => {
  ...
  // Fragment shader source code - Blue gradient theme for buttons
  const fsSource = `
    ...
    const vec4 lineColor = vec4(0.3, 0.5, 0.9, 1.0);
    ...
    void main() {
      ...
      vec4 lines = vec4(0.0);
      vec4 bgColor1 = vec4(0.15, 0.2, 0.4, 1.0);
      vec4 bgColor2 = vec4(0.25, 0.35, 0.55, 1.0);
      ...
      for(int l = 0; l < linesPerGroup; l++) {
        ...
        float line = drawSmoothLine(linePosition, halfWidth, space.y) / 2.0 + drawCrispLine(linePosition, halfWidth * 0.2, space.y);
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      if (container) {
        canvas.width = width || container.clientWidth || 200
        canvas.height = height || container.clientHeight || 50
      } else {
